...
/** Obtiene las familias de fuentes reales inyectadas por Next.js en el DOM */
function getComputedNextFonts(): { display: string; serif: string; sans: string } {
  ...
  spanDisplay.className = 'font-display'
  document.body.appendChild(spanDisplay)
  const display = getComputedStyle(spanDisplay).fontFamily || '"Bebas Neue", sans-serif'
  document.body.removeChild(spanDisplay)

  const spanSerif = document.createElement('span')
  spanSerif.className = 'font-serif'
  document.body.appendChild(spanSerif)
  const serif = getComputedStyle(spanSerif).fontFamily || '"Cormorant Garamond", Georgia, serif'
  document.body.removeChild(spanSerif)

  const spanSans = document.createElement('span')
  spanSans.className = 'font-sans'
  document.body.appendChild(spanSans)
  const sans = getComputedStyle(spanSans).fontFamily || '"Space Grotesk", sans-serif'
  document.body.removeChild(spanSans)

  return { display, serif, sans }
}
...
export function InstagramStoryModal({ platillo, onClose }: InstagramStoryModalProps) {
  ...
  const renderCanvasStory = useCallback(async () => {
    ...
    try {
      ...
      const fonts = getComputedNextFonts()
      ...
      if (activeTemplate === 'clasico') {
        ...
        ctx.textAlign = 'center'
        const logoY = 1145
        ctx.font = `900 106px ${fonts.display}`
        ...
        ctx.font = `bold 38px ${fonts.display}`
        ...
        ctx.font = `italic 28px ${fonts.serif}`
        ...
        ctx.textAlign = 'center'
        ctx.fillStyle = '#F7F3EE'
        ctx.font = `900 78px ${fonts.display}`
        ...
        if (platillo.descripcion && platillo.descripcion.trim()) {
          ...
          ctx.font = `italic 34px ${fonts.sans}`
        ...
        ctx.font = `900 92px ${fonts.display}`
      ...
      // ═══════════════════════════════════════════════════════════════
        ...
        ctx.fillStyle = '#E8430A'
        ctx.font = `bold 26px ${fonts.display}`
        ...
        // Nombre del Platillo
        ctx.fillStyle = '#F7F3EE'
        ctx.font = `900 82px ${fonts.display}`
      ...
      // ═══════════════════════════════════════════════════════════════
        ...
        ctx.fillStyle = '#F7F3EE'
        ctx.font = `900 46px ${fonts.display}`
        ctx.textAlign = 'center'
        ctx.textBaseline = 'middle'
        ...
        ctx.textAlign = 'center'
        ctx.fillStyle = '#F7F3EE'
        ctx.font = `900 90px ${fonts.display}`
        ...
        ctx.textAlign = 'left'
        ctx.fillStyle = '#FFA800'
        ctx.font = `900 44px ${fonts.display}`
        ctx.fillText('$', curPX, pFireY + 82)

        ctx.fillStyle = '#FF5500'
        ctx.font = `900 100px ${fonts.display}`
      ...
      // ═══════════════════════════════════════════════════════════════
        ...
        ctx.textAlign = 'center'
        ctx.fillStyle = '#C9A84C'
        ctx.font = `bold 30px ${fonts.serif}`
        ctx.letterSpacing = '6px'
        ctx.fillText('— ✦ MARISCOS FRESCOS ✦ —', midX, 115)
        ctx.letterSpacing = '0px'

        ctx.fillStyle = '#F7F3EE'
        ctx.font = `900 88px ${fonts.display}`
        ...
        ctx.textAlign = 'center'
        ctx.fillStyle = '#F7F3EE'
        ctx.font = `900 84px ${fonts.display}`
        ...
        ctx.textAlign = 'center'
        ctx.fillStyle = '#FFA800'
        ctx.font = `900 36px ${fonts.display}`
      ...
      <div className="flex flex-col lg:flex-row items-center lg:items-start justify-center gap-6 w-full max-w-4xl my-auto">
        ...
        <div className="flex flex-col items-center gap-3 w-full lg:w-auto">
          ...
          <p className="text-[10px] text-arena/40 font-sans text-center max-w-[340px]">
            Renderizado por GPU nativo en 1080×1920 px con tipografía Bebas Neue y Space Grotesk.
          </p>
        </div>
        ...
        <div className="flex flex-col gap-4 w-full max-w-[340px] lg:w-80 shrink-0">
          ...
          <div className="bg-carbon border border-arena/10 rounded-xl p-4 flex flex-col gap-2.5">
            ...
            <div className="flex justify-between items-center text-xs font-sans">
              ...
              <span className="text-turquesa font-bold">iOS / Safari / Android 100%</span>
            </div>
          </div>
